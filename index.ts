...
function importQuery(): void {
  try {
    let textAreacontent: any = document.getElementById(
      'dlgContent'
...
function updateRule(): void {
  let validRule: any = qryBldrObj.getValidRules(qryBldrObj.rule);
  content = JSON.stringify(validRule, null, 4);
  (document.getElementById('json-texarea') as HTMLTextAreaElement).value =
    content;
  (
    document.getElementById('json-texarea') as HTMLTextAreaElement
  ).readOnly = true;
}
document.getElementById('json-btn').onclick = (): void => {
  dialogObj.show();
};
const queryPreview: any = document.getElementsByClassName(
